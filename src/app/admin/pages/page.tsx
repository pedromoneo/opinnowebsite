...
export default function AdminPages() {
    ...
    useEffect(() => {
        const fetchPages = async () => {
            try {
                // Fetch latest 100 pages for the dashboard simply
                ...
                const fetchedPages: any[] = snapshot.docs.map(doc => ({
                    id: doc.id,
                    ...doc.data()
                }))
    ...
    return (
        <div className="flex flex-col gap-6">
            <header className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-display font-bold text-opinno-primary tracking-tight">Pages</h1>
                    <p className="text-gray-500 mt-1">Manage static pages across the site.</p>
                </div>
                <Link href="/admin/pages/new" className="px-5 py-2.5 bg-opinno-accent text-white rounded-lg font-medium hover:bg-opinno-accent-hover transition-colors shadow-sm flex items-center gap-2">
                            ...
                            {loading ? (
                                ...
                                <tr key={page.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="text-sm font-medium text-gray-900 max-w-md truncate">{page.title}</div>
                                        <div className="text-xs text-gray-500 truncate">/{page.lang}/{page.slug}</div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {page.publishedAt ? new Date(page.publishedAt).toLocaleDateString() : 'Draft'}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link href={`/admin/pages/${page.id}`} className="text-opinno-accent hover:text-opinno-accent-hover mr-4">Edit</Link>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
